...
try:
    import Queue as queue  # Python 2 # type: ignore
except ImportError:
...
class AbletonMCP(ControlSurface):
    ...
    def _process_command(self, command):
        ...
        self.log_message(f"--->>> _process_command START for: {command_type}")
        ...
        try:
            ...
            # Commands that modify Live's state should be scheduled on the main thread
                ...
                self.log_message(f"--->>> Scheduling modifying command: {command_type}")
                ...
                def main_thread_task():
                    self.log_message(
                        f"--->>> main_thread_task START for: {command_type}"
                    )
                    try:
                        result = None
                        if command_type == "create_midi_track":
                            ...
                            try:
                                ...
                                self.log_message(
                                    f"Error directly calling _delete_track: {str(inner_e)}"
                                )
                                response_queue.put(
                                    {
                                        "status": "error",
                                        "message": f"Failed in _delete_track: {str(inner_e)}",
                                    }
                                )
                                return  # Prevent putting default success message
                            ...
                            try:
                                ...
                                self.log_message(
                                    f"Error directly calling _delete_clip: {str(inner_e)}"
                                )
                                response_queue.put(
                                    {
                                        "status": "error",
                                        "message": f"Failed in _delete_clip: {str(inner_e)}",
                                    }
                                )
                                return  # Prevent putting default success message

                        # Put the result in the queue (only if no exception occurred above)
    ...
    def _delete_track(self, track_index):
        ...
        self.log_message(f"---> Entering _delete_track for index: {track_index}")
        self.log_message(f"Attempting to delete track at index: {track_index}")
        try:
            num_tracks = len(self._song.tracks)
            if track_index < 0 or track_index >= num_tracks:
                raise IndexError(
                    f"Track index {track_index} out of range (0-{num_tracks - 1})"
                )

            # Check if the track exists before trying to delete
            if self._song.tracks[track_index]:
                self._song.delete_track(track_index)
                self.log_message(f"Successfully deleted track at index {track_index}")
                return {"message": f"Track {track_index} deleted successfully."}
            else:
                # This case might not be strictly necessary if index check is robust
                # but added for safety.
                raise Exception(
                    f"Track at index {track_index} does not exist or is invalid."
                )
            ...
            self.log_message(f"Unexpected error deleting track {track_index}: {str(e)}")
            self.log_message(traceback.format_exc())
            raise Exception(f"Failed to delete track {track_index}: {str(e)}")

    def _delete_clip(self, track_index, clip_index):
        """Delete a clip at the specified track and clip slot index."""
        self.log_message(
            f"---> Entering _delete_clip for track {track_index}, clip {clip_index}"
        )
        self.log_message(
            f"Attempting to delete clip at track {track_index}, slot {clip_index}"
        )
        try:
            num_tracks = len(self._song.tracks)
            if track_index < 0 or track_index >= num_tracks:
            ...
            track = self._song.tracks[track_index]
            num_clip_slots = len(track.clip_slots)
            if clip_index < 0 or clip_index >= num_clip_slots:
                raise IndexError(
                    f"Clip slot index {clip_index} out of range (0-{num_clip_slots - 1}) for track {track_index}"
                )
            ...
            if not clip_slot.has_clip:
                self.log_message(
                    f"No clip found at track {track_index}, slot {clip_index}. Nothing to delete."
                )
                return {
                    "message": f"No clip found at track {track_index}, slot {clip_index}. Nothing to delete."
                }

            clip_slot.delete_clip()

            self.log_message(
                f"Successfully deleted clip at track {track_index}, slot {clip_index}"
            )
            return {
            ...
            self.log_message(f"Error deleting clip: {str(e)}")
            raise IndexError(str(e))
        except Exception as e:
            self.log_message(
                f"Unexpected error deleting clip at track {track_index}, slot {clip_index}: {str(e)}"
            )
            self.log_message(traceback.format_exc())
            raise Exception(
